...
dataMean = mean(dataAll);
dataStd = std(dataAll);
dataScaled = (dataAll - dataMean) ./ dataStd;
dataScaled(isnan(dataScaled)) = 0;

%% classify (LDA)
group = ones(size(dataAll,1),1);
group(23:33)=2;


[class,err,POSTERIOR,logp,coeff] = classify(dataAll,dataAll,group);

%% pca

[coef, score, ~, ~, explained, ~] = pca(dataScaled); %, "Algorithm","eig" 'VariableWeights','variance'
% kmeans(dataScaled,2)
